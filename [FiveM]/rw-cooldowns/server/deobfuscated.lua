...
function TimeToString(time)
	...
	if Config.Debug then
		print(string.format('^3[COOLDOWNS]^7 TimeToString(%s) formattedDate: %s; final string: %s', time, formattedDate, str))
	end
...
function TimeToTable(time)
	...
	if Config.Debug then
		print(string.format('^3[COOLDOWNS]^7 TimeToTable(%s) table: %s', time, json.encode(tab)))
	end
	...
	DoesCooldownExist(function(doesExist)
		if doesExist then
			MySQL.Async.execute('UPDATE `cooldowns` SET `cooldown` = @cooldown WHERE `type` = @type', {
				...
				if Config.Debug then
					print(string.format('^3[COOLDOWNS]^7 The cooldown %s has been established to %s', type, cooldown))
				end
			end)
		else
			MySQL.Async.insert('INSERT INTO `cooldowns` (`type`, `cooldown`) VALUES (@type, @cooldown)', {
				...
				Cooldowns[type] = cooldown
				if Config.Debug then
					print(string.format('^3[COOLDOWNS]^7 The cooldown %s has been established to %s, creating a new entry with ID %s', type, cooldown, id))
				end
			end)
		end
	end, type)
...
-- 	RegisterNetEvent('rw-cooldowns:setCooldown')
-- 	AddEventHandler('rw-cooldowns:setCooldown', function(type, cooldown)
-- 		if Config.Debug then
-- 			print(string.format('^1[COOLDOWNS]^7 setCooldown event has been called by source %s', source or 'resource'))
-- 		end
...
AddEventHandler('rw-cooldowns:getCooldown', function(cb, type, format)
	if Config.Debug then
		print(string.format('^3[COOLDOWNS]^7 getCooldown event has been called by source %s', source or 'resource'))
	end
...
ESX.RegisterServerCallback('rw-cooldowns:getCooldown', function(source, cb, type, format)
	if Config.Debug then
		print(string.format('^3[COOLDOWNS]^7 getCooldown ESX callback has been called by source %s', source or 'resource'))
	end
...
AddEventHandler('rw-cooldowns:getTime', function(cb, type, format)
	if Config.Debug then
		print(string.format('^3[COOLDOWNS]^7 getTime event has been called by source %s', source or 'resource'))
	end
...
ESX.RegisterServerCallback('rw-cooldowns:getTime', function(source, cb, type, format)
	if Config.Debug then
		print(string.format('^3[COOLDOWNS]^7 getTime ESX callback has been called by source %s', source or 'resource'))
	end
