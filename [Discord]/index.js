...
if (+process.version.slice(1).split(".")[0] < 12) {
  console.log(
    "\u001b[31m Bot requires Node JS version 12 or higher. Please go to https://nodejs.org/en/ then download and install the LTS version.\033[0m"
  );
  process.exit();
}
(async () => {
  ...
  try {
    ...
    if (["YAMLSemanticError", "YAMLSyntaxError"].includes(e.name))
      console.log(
        Utils.errorPrefix +
          "Konfiguratsiooni- või lang-faili laadimisel ilmnes viga. Bot lülitub kinni..." +
          Utils.color.Reset
      );
    else console.log(e);
  ...
  Utils.yml("./config.yml").then((config) => {
    bot.login(config.Token).catch((error) => {
      console.log(Utils.errorPrefix + "Teie boti tunnus on vale! Sulgun...");
      process.exit();
    });
    variables.set("bot", bot);
  });
