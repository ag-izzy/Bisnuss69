...
module.exports = async (bot) => {
  ...
  const botStatus = (await variables.db.get.getStatus()) || {
    activity: "DTeam",
    type: "WATCHING",
  };
  if (config.ActivityCycling.Enabled) {
    ...
    async function nextStatus() {
      ...
      if (config.ActivityCycling.Type == "ordered") {
        ...
        pos++;
      } else if (config.ActivityCycling.Type == "random")
        status =
          config.ActivityCycling.Activities[
            Math.floor(Math.random() * config.ActivityCycling.Activities.length)
          ];

      if (Array.isArray(status)) {
        if (status.length == 2)
          await variables.db.update.status.setStatus(status[0], status[1]);
  ...
  console.log(
    "\x1b[32m",
    `                         CoreBot v${variables.config.BotVersion} is now ONLINE!`
  );
  console.log(
    "\x1b[36m",
    `                       Thank you for purchasing CoreBot!`
  );
  console.log(" ");
  console.log(
    `${chalk.hex("#ffc042")(
  ...
  const GiveawayHandler =
    await require("../modules/handlers/GiveawayHandler.js")(bot);
  ...
  if (
    ...
    variables.config.Key == "BOT-KEY-HERE"
  ) {
    console.log(
      "\x1b[91m%s\x1b[0m",
      "[WARNING] CoreBot is not activated. Please make sure you have put your key into the Key section in the config. If it is, then corebot will activate within the next few seconds."
    );
  }
  ...
  // Backup System
  if (
    variables.config.Other.SQLiteDatabaseBackups &&
    !Utils.getStartupParameters().includes("disable-backups")
  ) {
    function backup() {
  ...
  const StatusAdvertisementHandler =
    await require("../modules/handlers/StatusHandler.js")(bot);

  if (config.Logs.Enabled.includes("Chat") && !fs.existsSync("./logs/"))
    await fs.mkdirSync("./logs/", function (err) {
      if (err) throw err;
    });
  ...
  if (
    ...
    bot.guilds.cache.forEach((guild) => {
      ...
      guild.channels.cache
        .filter((ch) => ch.permissionsFor(muteRole).has("SEND_MESSAGES"))
        .forEach((ch) => {
  ...
  if (bot.guilds.cache.size == 1) {
    ...
    console.log(
      Utils.warningPrefix +
        "Your bot is currently in " +
        chalk.bold(0) +
        " servers! This may cause errors in console. Please invite the bot to your server!"
    );
  } else {
    console.log(
      Utils.warningPrefix +
        "Your bot is currently in " +
        chalk.bold(bot.guilds.cache.size) +
        " servers! Corebot is not made for multiple servers, so there may be various bugs."
    );
  }
    ...
    config.AutoAnnouncements.Announcements.forEach((announcement) => {
      let announcer = async () => {
        ...
        if (announcement.Type == "embed" && announcement.Embed) {
          ...
          if (
            lastMessage &&
            lastMessage.embeds.length &&
            lastMessage.author.id == bot.user.id
          ) {
            if (
              lastMessage.embeds[0].title &&
              embed.title &&
              embed.title == lastMessage.embeds[0].title
            )
              return;
            if (
              lastMessage.embeds[0].description &&
              embed.description &&
              embed.description == lastMessage.embeds[0].description
            )
              return;
          }
          ...
          if (
            lastMessage &&
            lastMessage.content == announcement.Content &&
            lastMessage.author.id == bot.user.id
          )
            return;
  ...
  let SuggestCMD = CommandHandler.find(
    (command) => command.command == "suggest"
  );
  let SNoteCMD = CommandHandler.find((command) => command.command == "snote");
  let BugReportsCMD = CommandHandler.find(
    (command) => command.command == "bugreport"
  );
  let TempChannelCMD = CommandHandler.find(
    (command) => command.command == "tempchannel"
  );
  ...
  if (SuggestCMD) {
    if (
      ...
      (config.Suggestions.Enabled &&
        config.Suggestions.Type.toLowerCase() == "send-message")
    ) {
  ...
  if (BugReportsCMD) {
    if (
      !config.BugReports.Enabled ||
      (config.BugReports.Enabled &&
        config.BugReports.Type.toLowerCase() == "send-message")
    ) {
