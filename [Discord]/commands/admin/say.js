...
module.exports = {
  ...
  run: async (bot, message, args, { prefixUsed, commandUsed }) => {
    ...
    if (
      ...
      !["embed", "normal", "advanced"].includes(action)
    )
    ...
    const channel =
      Utils.ResolveChannel(message, 1, false, false) || message.channel;
    ...
    if (action == "normal") return channel.send(msg);
      ...
      return channel.send(Embed({ description: msg }));
      ...
      msg
        ...
        .forEach((property) => {
          ...
          if (key == "author") {
            let startOfName =
              value.indexOf("name=") == -1 ? undefined : value.indexOf("name=");
            let startOfIcon =
              value.indexOf("icon=") == -1 ? undefined : value.indexOf("icon=");
            let startOfURL =
            ...
            if (value == "bot")
              value = bot.user.displayAvatarURL({ dynamic: true });
      ...
      if (!embed.color)
        embed.color = Utils.variables.config.EmbedColors.Default;
