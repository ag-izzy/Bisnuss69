...
module.exports = {
  ...
  run: async (bot, message, args, { prefixUsed, commandUsed }) => {
    ...
    let content = message.content
      .replace(prefixUsed + commandUsed, "")
      .replace(
        /(<@[0-9]{18}>|<@![0-9]{18}>|<@&[0-9]{18}>|users|tickets)\s+(normal|embed|)/g,
        ""
      );

    if (!["normal", "embed"].includes(args[1].toLowerCase()))
      return message.channel.send(
        Embed({ preset: "invalidargs", usage: module.exports.usage })
      );
        ...
        channel.send(content).catch((err) => {
          if (sendError)
            message.channel.send(
              Embed({
                preset: "error",
                description: lang.AdminModule.Commands.Msg.CouldntSend,
              })
            );
          return false;
        });
      else if (args[1].toLowerCase() == "embed")
        channel.send(Embed({ description: content })).catch((err) => {
          if (sendError)
            message.channel.send(
              Embed({
                preset: "error",
      ...
      if (sent)
        await message.channel.send(
          Embed({
            ...
            title: lang.AdminModule.Commands.Msg.Sent,
          })
        );
      return;
    }
    ...
    if (msgRole) {
      let members = message.guild.members.cache.filter((u) =>
        u.roles.cache.has(msgRole.id)
      );
