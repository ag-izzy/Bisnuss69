...
module.exports = {
  ...
  run: async (bot, message, args) => {
    ...
    const askQuestion = async (i, ask = true) => {
      ...
      if (ask)
        await message.channel
          .send(
            Embed({
              title:
                lang.AdminModule.Commands.Announce.AnnouncementSetup.replace(
                  /{pos}/g,
                  i + 1 + "/5"
                ),
      ...
      await Utils.waitForResponse(message.author.id, message.channel).then(
        (response) => {
          ...
          if (response.content.toLowerCase() === "cancel")
            return message.channel.send(
              Embed({
                preset: "error",
                description: lang.AdminModule.Commands.Announce.SetupCanceled,
              })
            );
          else if (i == 2) {
            if (response.mentions.channels.first()) {
              ...
              if (response.content == "here") answers.push(message.channel);
                ...
                let channel = Utils.findChannel(
                  config.Channels.DefaultAnnouncements,
                  message.guild
                );
                  ...
                  message.channel
                    .send(
                      Embed({
                        ...
                        title:
                          lang.AdminModule.Commands.Announce.Embeds
                            .InvalidChannel.Title,
                        description:
                          lang.AdminModule.Commands.Announce.Embeds
                            .InvalidChannel.Description,
                      })
                    )
                ...
                message.channel
                  .send(
                    Embed({
                      color: config.EmbedColors.Error,
                      title:
                        lang.AdminModule.Commands.Announce.Embeds.InvalidChannel
                          .Title,
                      description:
                        lang.AdminModule.Commands.Announce.Embeds.InvalidChannel
                          .Description,
            ...
            if (response.content.toLowerCase() == "everyone")
              toTag = "@everyone";
            if (!!response.mentions.roles.first())
              toTag = response.mentions.roles.map((r) => r.id);
            if (
              response.content
                .toLowerCase()
                .replace(/\s+/g, "")
                ...
                .some(
                  (rolename) =>
                    !!response.guild.roles.cache.find(
            ...
            if (typeof toTag == "object" && toTag.length < 1)
              toTag == undefined;
    ...
    const finishAnnouncement = async () => {
      if (toTag && typeof toTag == "string")
        answers[2].send(toTag).then((msg) => msg.delete({ timeout: 1500 }));
      if (toTag && typeof toTag == "object" && toTag.length > 0)
        answers[2]
          .send(toTag.map((id) => "<@&" + id + ">").join(", "))
      ...
      msgIDs.forEach(async (id) =>
        (await message.channel.messages.fetch(id)).delete()
      );
      message.channel.send(
        Embed({
          title: lang.AdminModule.Commands.Announce.Embeds.Posted.Title,
          description:
            lang.AdminModule.Commands.Announce.Embeds.Posted.Description,
