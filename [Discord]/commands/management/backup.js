  ...
  run: async (bot, message, args) => {
          ...
          if (!restore)
            message.channel.send(
              Embed({
                title: "Data Saved",
                description:
                  ":white_check_mark: The Discord server was successfully backed up to **" +
                  file +
                  "**",
              })
            );
    ...
    async function restore() {
      ...
      const latest = require("../../server_backups/" +
        fs
          .readdirSync("./server_backups/")
          .map((f) => f.replace(".json", ""))
          .filter((f) => !f.startsWith("restore_"))
          .sort((a, b) => now - a - (now - b))[0]);

      if (!latest)
        return message.channel.send(
          Embed({ preset: "error", description: "There are no backups" })
        );
        ...
        .then(async (m) => {
          ...
          let confirmation = await message.channel.send(
            Embed({
              color: "#ffa600",
              title:
                "Are you sure you would like to restore this data? All the current roles, channels, and/or emojis will be deleted then re-created.",
            })
          );
          ...
          let confirmationEmoji = await Utils.waitForReaction(
            ["❌", "✅"],
            message.member.id,
            confirmation
          );
          ...
          let restoreChannels = async () => {
            return new Promise(async (resolve) => {
              ...
              setTimeout(async function () {
                await latest.channels
                  .filter((c) => c.type == "category")
                  .concat(latest.channels.filter((c) => c.type !== "category"))
                  .forEach((channel, i) => {
                    log("Creating channel: " + channel.name);
                    message.guild.channels
                      .create(channel.name, { type: channel.type })
                      .catch((err) => {
                        log("Could not create channel" + channel.name);
                      })
                      .then((ch) => {
                        if (channel.parent && ch && channel.type !== "category")
                          ch.setParent(
                            Utils.findChannel(
                              channel.parent,
                              message.guild,
          ...
          if (type == emojis[0]) {
            ...
            if (message.guild.channels.cache.first())
              message.guild.channels.cache.first().send(
                Embed({
                  title: "Restored All Data",
                  description:
                    "A log of what was changed can be viewed here: " + paste,
    ...
    switch (type) {
      ...
      default:
        message.channel
          .send(
            Embed({
              title: "Would you like to save data or restore data?",
          ...
          .then(async (m) => {
            ...
            let reaction = await Utils.waitForReaction(
              ["🇷", "🇸"],
              message.member.id,
              m
            );
