...
module.exports = {
  ...
  run: async (bot, message, args, { prefixUsed, commandUsed }) => {
    if (!commands.Permissions.reset.includes(message.member.id))
      return message.channel.send(Embed({ preset: "nopermission" }));
    if (args.length < 1)
      return message.channel.send(
        Embed({ preset: "invalidargs", usage: module.exports.usage })
      );
    ...
    else if (["xp", "exp", "experience"].includes(type))
      tables.push("experience");
    ...
    message.channel
      .send(Embed({ title: lang.ManagementModule.Commands.Reset.Confirmation }))
      .then((m) => {
        m.react("✅");
        m.react("❌");

        Utils.waitForReaction(["✅", "❌"], message.member.id, m).then(
          async (reaction) => {
            m.delete();
            ...
            if (reaction.emoji.name == "❌") {
              ...
              if (config.Storage.Type == "sqlite")
                await Utils.backup(["database.sqlite"]);
              console.log(
                Utils.backupPrefix +
                  "Filed salvestatud " +
              ...
              console.log(
                Utils.warningPrefix +
                  "Järgnevad tableid on resetimises: " +
                  tables.join(", ")
              );
              let msg = await message.channel.send(
                Embed({ title: "Resetting andmeid..." })
              );
                ...
                .then(() => {
                  msg.edit(
                    Embed({
                      ...
                      description:
                        lang.ManagementModule.Commands.Reset.Embed.Description.replace(
                          /{tables}/g,
                          tables.map((t) => `\`${t}\``).join(", ")
                        ),
                      timestamp: new Date(),
  ...
  usage:
    "reset <coins/xp/tickets/giveaways/punishments/warnings/applications/jobs/daily/games/saved roles>",
  aliases: ["clean"],
};
